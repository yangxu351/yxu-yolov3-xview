...
#    ap_list = [20, 40, 50]
    ...
    for cx, cc_id in enumerate(ccids):
        ...
        for ix, cbs in enumerate(cc_ratios):
            ...
            hyp_cmt = hyp_str.format(opt.cc_id, cbs, opt.batch_size-cbs)
...
#        hyp_cmt = hyp_str.format(cc_id, cbs, bkgbs)
    ...
    #        bkgbs = 8-cbs
        ...
        cmb = np.array(cmb)
        avg_cmb = np.mean(cmb[:, :, 5:9], axis=0)
        #df_avg = pd.DataFrame(columns=["Version", "CC", "CC_ratio", "Nx_bkg", "Seen", "NT", "AP{}".format(apN), "Pd(FAR=0.25)",  "Pd(FAR=0.5)", "Pd(FAR=1)"])
        df_avg = pd.DataFrame(columns=["Version", "CC", "CC_ratio", "Seen", "NT", "AP{}".format(apN), "Pd(FAR=0.25)",  "Pd(FAR=0.5)", "Pd(FAR=1)"])
