...
#    ap_list = [20, 40, 50]
    ...
    for cc_id in ccids:
        for cbs in cc_ratios:
            ...
            for ix, times in enumerate(times_list):
                ...
                hyp_cmt = hyp_str.format(cbs, opt.batch_size-cbs, opt.cc_id, times)
    ...
    #                suffix = ''
                ...
                opt.legend = prefix
                opt.name = prefix # 'syn_CC*_v*_dyn*_color_bias*_AP*
                ...
                opt.result_dir = os.path.join(opt.result_dir.format(opt.class_num, base_cmt), 'test_on_xview_rcncc_bkg_cc{}_{}_iou{}_seed{}'.format(hyp_cmt, opt.type, apN, seed))
                opt.data = 'data_xview/{}_cls/{}/CC/xview_rcncc_bkg_cc{}_test_{}.data'.format(opt.class_num, base_cmt, opt.cc_id, base_cmt)
    
                if not os.path.exists(opt.result_dir):
                    os.makedirs(opt.result_dir)
                print(os.path.join(opt.weights_dir.format(opt.class_num, base_cmt, sd), "*_{}_seed{}".format(hyp_cmt, seed), "best_seed{}.pt".format(seed)))
    ...
    #            all_weights = glob.glob(os.path.join(opt.weights_dir.format(opt.class_num, cmt, sd), "*_{}_seed{}".format(hyp_cmt, sd), "best_*seed{}.pt".format(sd)))
                ...
                df_pr_ap_far.at[ix, "CC"] = opt.cc_id
                df_pr_ap_far.at[ix, "CC_ratio"] = cbs
                df_pr_ap_far.at[ix, "Nx_bkg"] = times
    ...
    #            print('idx25_mn', idx25_mn)
            ...
            csv_dir = "result_output/{}_cls/{}/syn_CC+xview_BG/{}/".format(opt.class_num, base_cmt, hyp_str.format(cbs, opt.batch_size-cbs, opt.cc_id, 'N'))
            if not os.path.exists(csv_dir):
                os.makedirs(csv_dir)
            
            csv_name =  "syn_cc{}+xview_bkg_cc{}bg{}_{}_seed{}.xlsx".format(opt.cc_id, cbs, opt.batch_size-cbs, opt.type, seed)          
            mode = 'w'
...
def computer_avg_all_seeds(seeds):
    ...
    for cc_id in ccids:
        for ix, times in enumerate(times_list):
            ...
            csv_dir = "result_output/1_cls/{}/syn_CC+xview_BG/{}/".format(base_cmt, hyp_cmt)
            
            cmb = []
            for seed in seeds:
                csv_name =  "{}_cc{}bg{}_{}_seed{}.xlsx".format(prefix, cbs, bkgbs, eht, seed)          
                ...
                cc_list = df.loc[:, "CC"]
                cbs_list = df.loc[:, "CC_ratio"]
                nx_list = df.loc[:, "Nx_bkg"]
                imgnum_list = df.loc[:, 'Seen']
                CCnum_list = df.loc[:, 'NT']
                
            cmb = np.array(cmb)
            avg_cmb = np.mean(cmb[:, :, 6:10], axis=0)
            df_avg = pd.DataFrame(columns=["Version", "CC", "CC_ratio", "Nx_bkg", "Seen", "NT", "AP{}".format(apN), "Pd(FAR=0.25)",  "Pd(FAR=0.5)", "Pd(FAR=1)"])
            df_avg['Version'] = ver_list
            df_avg['CC'] = cc_list
            df_avg['CC_ratio'] = cbs_list
            df_avg['Nx_bkg'] = nx_list
            df_avg['Seen'] = imgnum_list
            df_avg['NT'] = CCnum_list
            
            df_avg.loc[:, 6:10] = avg_cmb
            save_name =  "{}_cc{}bg{}_{}_avg_all_seeds.xlsx".format(prefix, cbs, bkgbs, eht, seed)
