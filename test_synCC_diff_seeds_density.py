...
#                        print('ni ', len(ni), ni) print('ni ', len(ni), ni)
                    else:
                        ni = torch.tensor([])
...
#    ap_list = [20, 40, 50]
    ...
    df_pr_ap_far = pd.DataFrame(columns=["Version", "Seen", "NT", "Nx_density", "AP{}".format(apN), "Pd(FAR=0.25)",  "Pd(FAR=0.5)", "Pd(FAR=1)", "Precision", "Recall" , "F1"])
    for ix, cmt in enumerate(comments):
        times = density_times[ix]
        VN = ix + base_version
        ...
        opt.name = mstr + suffix # 'syn_CC*_v*_dyn*_color_bias*_AP* prefix + 
        ...
        opt.result_dir = os.path.join(opt.result_dir.format(opt.class_num, cmt, sd), 'test_on_xview_rcncc_bkg_cc{}_{}_iou{}_seed{}'.format(hyp_cmt, opt.type, apN, seed))
...
#            all_weights = glob.glob(os.path.join(opt.weights_dir.format(opt.class_num, cmt, sd), "*_{}_seed{}".format(hyp_cmt, sd), "best_*seed{}.pt".format(sd)))
        ...
        df_pr_ap_far.at[ix, "Nx_density"] = times
...
#            print('idx25_mn', idx25_mn)
    ...
    csv_dir = "result_output/{}_cls/syn_CC/{}_Nxdensity/".format(opt.class_num, cmt[:cmt.find("_CC")+4])
...
def computer_avg_all_seeds(seeds):
    ...
    for ix, cmt in enumerate(comments):
        ...
        csv_dir = "result_output/1_cls/syn_CC/{}_Nxdensity/".format(cmt[:cmt.find("_CC")+4])
        prefix = 'synCC{}_Nxdensity'.format(cc_id)
        times = density_times[ix]
        cmb = []
        for seed in seeds:
            csv_name =  "{}-seed{}.xlsx".format(prefix, seed)
            ...
            times_list = df.loc[:, 'Nx_density']

        cmb = np.array(cmb)
        avg_cmb = np.mean(cmb[:, :, 4:8], axis=0)
        df_avg = pd.DataFrame(columns=["Version", "Seen", "NT", "Nx_density", "AP{}".format(apN), "Pd(FAR=0.25)",  "Pd(FAR=0.5)", "Pd(FAR=1)"])
        df_avg['Version'] = ver_list
        df_avg['Seen'] = imgnum_list
        df_avg['NT'] = CCnum_list
        df_avg['Nx_density'] = times_list
...
if __name__ == "__main__":
    ...
    for ix, times in enumerate(density_times):
        #cmt = 'syn_xview_bkg_px23whr3_new_bkg_shdw_split_scatter_gauss_rndsolar_promu_size_square_bias0.12_CC2_{}xdensity_v{}'.format(times, ix+50)
        cmt = 'syn_xview_bkg_px23whr3_new_bkg_unif_shdw_split_scatter_gauss_rndsolar_ssig0.24_color_square_bias2_CC1_{}xdensity_v{}'.format(times, ix+50)
