...
def get_opt(dt=None, sr=None, comments=''):
    ...
    parser.add_argument("--weights", type=str, default="weights/{}_cls/syn_CC+xview_BG/{}_{}/best_{}_{}.pt", help="path to weights file")
...
def main(seed, device):
    ...
    hyp_str = 'hgiou1_19.5obj_cc6bg2_ccid{}_{}inst'
    #hyp_str1 = 'hgiou1_19.5obj_cc4bg4_ccid{}_{}inst'
    hyp_str1 = 'hgiou1_19.5obj_cc7bg1_ccid{}_{}inst'
    hyp_str2 = 'hgiou1_19.5obj_cc7bg1_ccid{}_{}inst'
    prefix_str = 'synCC{}_{}inst'
    #`t3prefix_str2 = 'synCC{}_ignore_notsolid_Nxinstance'
...
#    ap_list = [20, 40, 50]
    ...
    df_pr_ap_far = pd.DataFrame(columns=["Version", "Seen", "NT", "Nx_instance", "AP{}".format(apN), "Pd(FAR=0.25)",  "Pd(FAR=0.5)", "Pd(FAR=1)", "Precision", "Recall" , "F1"])
    for ix, times in enumerate(times_list):
        ...
        if opt.cc_id == 1:
            hyp_cmt = hyp_str1.format(opt.cc_id, times, seed)
        else:
            hyp_cmt = hyp_str2.format(opt.cc_id, times, seed)
        #hyp_cmt = hyp_str.format(opt.cc_id, times, seed)
        prefix = prefix_str.format(opt.cc_id, times) 
        
#        if opt.cc_id ==2:
#            prefix = prefix_str2.format(opt.cc_id)    
#        else:
#            prefix = prefix_str.format(opt.cc_id)            
        
        opt.legend = prefix + suffix  # prefix + 
        opt.name = prefix + mstr + suffix # 'syn_CC*_v*_dyn*_color_bias*_AP* prefix + 
        ...
        opt.result_dir = os.path.join(opt.result_dir.format(opt.class_num, cmt), 'test_on_xview_rcncc_bkg_cc_{}_{}_iou{}_seed{}'.format(hyp_cmt, opt.type, apN, seed))
        #opt.result_dir = os.path.join(opt.result_dir.format(opt.class_num, cmt, sd), 'test_on_xview_rcncc_bkg_cc_ignore_notsolid_{}_{}_iou{}_seed{}'.format(hyp_cmt, opt.type, apN, seed))
        opt.data = 'data_xview/{}_cls/{}/CC/xview_rcncc_bkg_cc{}_test_{}.data'.format(opt.class_num, base_cmt, opt.cc_id, base_cmt)

        if not os.path.exists(opt.result_dir):
            os.makedirs(opt.result_dir)
        print(os.path.join(opt.weights_dir.format(opt.class_num, cmt), "*_{}_seed{}".format(hyp_cmt, seed), "best_seed{}.pt".format(seed)))
        all_weights = glob.glob(os.path.join(opt.weights_dir.format(opt.class_num, cmt), "*_{}_seed{}".format(hyp_cmt, seed), "best_*seed{}.pt".format(seed)))
        all_weights.sort()
        ...
        df_pr_ap_far.at[ix, "Nx_instance"] = times
...
#            print('idx25_mn', idx25_mn)
    ...
    csv_dir = "result_output/{}_cls/syn_CC+xview_BG/{}_Nxinstance/".format(opt.class_num, cmt[:cmt.find("_CC")+4])
    if not os.path.exists(csv_dir):
        os.makedirs(csv_dir)

    csv_name =  "synCC{}_Nxinstance-seed{}.xlsx".format(opt.cc_id, seed)          
    mode = 'w'
...
def computer_avg_all_seeds(seeds):
    #prefix = 'synCC{}_Nxinstance'
    eht = 'easy'
    apN = 50
    for ix, cmt in enumerate(comments):
        cinx = cmt.find('_CC')
        cc_id = int(cmt[cinx+3])
        csv_dir = "result_output/1_cls/syn_CC+xview_BG/{}_Nxinstance/".format(cmt[:cmt.find("_CC")+4])
#        if cc_id ==2:
#            prefix = 'synCC{}_ignore_notsolid_Nxquantity'.format(cc_id)
#        else:
#            prefix = 'synCC{}_Nxquantity'.format(cc_id)
        prefix = 'synCC{}_Nxinstance'.format(cc_id)
        #times = quantity_times[ix]
        cmb = []
        for seed in seeds:
            csv_name =  "{}-seed{}.xlsx".format(prefix, seed)
            ...
            times_list = df.loc[:, 'Nx_instance']

        cmb = np.array(cmb)
        avg_cmb = np.mean(cmb[:, :, 4:8], axis=0)
        df_avg = pd.DataFrame(columns=["Version", "Seen", "NT", "Nx_instance", "AP{}".format(apN), "Pd(FAR=0.25)",  "Pd(FAR=0.5)", "Pd(FAR=1)"])
        df_avg['Version'] = ver_list
        df_avg['Seen'] = imgnum_list
        df_avg['NT'] = CCnum_list
        df_avg['Nx_instance'] = times_list
        df_avg.loc[:, 4:8] = avg_cmb
        save_name =  "{}_avg_all_seeds.xlsx".format(prefix)
...
if __name__ == "__main__":
    ...
    #times_list = [6, 12, 18, 24, 30] 
    times_list = [7, 14, 21, 28, 35]
    #cmt = 'syn_xview_bkg_px23whr3_new_bkg_unif_shdw_split_scatter_gauss_rndsolar_ssig0_color_square_bias0_CC1_1inst_v63'
    #cmt = 'syn_xview_bkg_px23whr3_new_bkg_shdw_split_scatter_gauss_rndsolar_promu_size_square_bias0_CC2_1inst_v63'
    #cmt = 'syn_xview_bkg_px23whr3_new_bkg_unif_shdw_split_scatter_gauss_rndsolar_ssig0.24_color_square_bias2_CC1_1inst_v64'
    #cmt = 'syn_xview_bkg_px23whr3_new_bkg_shdw_split_scatter_gauss_rndsolar_promu_size_square_bias0.12_CC2_1inst_v64'
    cmt = 'syn_xview_bkg_px23whr3_new_bkg_unif_shdw_split_scatter_gauss_rndsolar_ssig0.08_csig1_CC1_1inst_v65'
    #cmt = 'syn_xview_bkg_px23whr3_new_bkg_shdw_split_scatter_gauss_rndsolar_promu_ssig0_csig4_CC2_1inst_v65'
...
#      main(seed, device)   
    ...
    seeds = [0, 1, 2]  # , 3, 4
#    comments = ['syn_xview_bkg_px23whr3_new_bkg_unif_shdw_split_scatter_gauss_rndsolar_ssig0.24_color_square_bias2_CC1_1inst_v64',
#    'syn_xview_bkg_px23whr3_new_bkg_shdw_split_scatter_gauss_rndsolar_promu_size_square_bias0.12_CC2_1inst_v64']
    comments = ['syn_xview_bkg_px23whr3_new_bkg_unif_shdw_split_scatter_gauss_rndsolar_ssig0.08_csig1_CC1_1inst_v65'] 
    #comments = ['syn_xview_bkg_px23whr3_new_bkg_shdw_split_scatter_gauss_rndsolar_promu_ssig0_csig4_CC2_1inst_v65']
