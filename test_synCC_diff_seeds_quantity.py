...
def main(seed, device):
    ...
    prefix_str = 'synCC{}_Nxquantity'
    prefix_str2 = 'synCC{}_ignore_notsolid_Nxquantity'
...
#    ap_list = [20, 40, 50]
    ...
    df_pr_ap_far = pd.DataFrame(columns=["Version", "Seen", "NT", "Nx_quantity", "AP{}".format(apN), "Pd(FAR=0.25)",  "Pd(FAR=0.5)", "Pd(FAR=1)", "Precision", "Recall" , "F1"])
...
#            all_weights = glob.glob(os.path.join(opt.weights_dir.format(opt.class_num, cmt, sd), "*_{}_seed{}".format(hyp_cmt, sd), "best_*seed{}.pt".format(sd)))
        ...
        df_pr_ap_far.at[ix, "Nx_quantity"] = times
...
#            print('idx25_mn', idx25_mn)
    ...
    csv_dir = "result_output/{}_cls/syn_CC/{}_Nxquantity/".format(opt.class_num, cmt[:cmt.find("_CC")+4])
...
def computer_avg_all_seeds(seeds):
    prefix = 'synCC{}_Nxquantity'
    eht = 'easy'
    apN = 50
    for ix, cmt in enumerate(comments):
        cinx = cmt.find('_CC')
        cc_id = int(cmt[cinx+3])
        csv_dir = "result_output/1_cls/syn_CC/{}_Nxquantity/".format(cmt[:cmt.find("_CC")+4])
#        if cc_id ==2:
...
#            prefix = 'synCC{}_Nxquantity'.format(cc_id)
        ...
        for seed in seeds:
            ...
            times_list = df.loc[:, 'Nx_quantity']

        cmb = np.array(cmb)
        avg_cmb = np.mean(cmb[:, :, 4:8], axis=0)
        df_avg = pd.DataFrame(columns=["Version", "Seen", "NT", "Nx_quantity", "AP{}".format(apN), "Pd(FAR=0.25)",  "Pd(FAR=0.5)", "Pd(FAR=1)"])
        df_avg['Version'] = ver_list
        df_avg['Seen'] = imgnum_list
        df_avg['NT'] = CCnum_list
        df_avg['Nx_quantity'] = times_list
