...
if __name__ == "__main__":
    ...
    xview
    '''
    cmt = 'xview_syn_xview_bkg_px23whr3_xbw_xcolor_xbkg_unif_shdw_split_scatter_gauss_rndsolar_dynsigma_size_bias0.15_RC5_v43_color'
    hyp_cmt = 'hgiou1_x4s4'
    ...
    for typ in eh_types:
        ...
        # df_pr_ap = pd.DataFrame(columns=["Version", "Seen", "NT", "AP{}".format(apN)])
        df_pr_ap_far = pd.DataFrame(columns=["RC", "Seen", "NT", "AP{}".format(apN), "Pd(FAR=0.5)", "Pd(FAR=1)", "Precision", "Recall" , "F1"]) #, "Precision", "Recall" , "F1"
...
#        csv_dir = "result_output/{}_cls/{}/".format(opt.class_num, cmt[:cmt.find("bias")+4] + cmt[cmt.find("_model"):cmt.find("_v")])
        ...
        bix = cmt.find('bias')    
        csv_dir = "result_output/{}_cls/{}_RC{}/".format(opt.class_num, cmt[:bix + 4], opt.rare_class)
