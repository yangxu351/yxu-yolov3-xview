...
def get_opt(dt=None, sr=None, comments=''):
    ...
    parser.add_argument("--weights", type=str, default="weights/{}_cls/xview_CC+xview_BG/{}_{}/best_{}_{}.pt", help="path to weights file")
    ...
    parser.add_argument("--weights_dir", type=str, default="weights/{}_cls/xview_CC+xview_BG/", help="to save weights path")
    parser.add_argument("--result_dir", type=str, default="result_output/{}_cls/xview_CC+xview_BG/", help="to save result files path")
...
#    ap_list = [20, 40, 50]
    ...
    for ix, times in enumerate(times_list):
        ...
        opt.legend = prefix # prefix + 
        opt.name = prefix  # 'syn_CC*_v*_dyn*_color_bias*_AP* prefix + 
        ...
        opt.result_dir = os.path.join(opt.result_dir.format(opt.class_num), 'test_on_xview_rcncc_bkg_cc_{}_{}_iou{}_seed{}'.format(hyp_cmt, opt.type, apN, seed))
        ...
        print(os.path.join(opt.weights_dir.format(opt.class_num), "*_{}_seed{}".format(hyp_cmt, seed), "best_seed{}.pt".format(seed)))
        all_weights = glob.glob(os.path.join(opt.weights_dir.format(opt.class_num), "*_{}_seed{}".format(hyp_cmt, seed), "best_*seed{}.pt".format(seed)))
        all_weights.sort()
...
#            print('idx25_mn', idx25_mn)
    ...
    csv_dir = "result_output/{}_cls/xview_CC+xview_BG/CC{}_Nxinstance/".format(opt.class_num, opt.cc_id)
    if not os.path.exists(csv_dir):
        os.makedirs(csv_dir)

    csv_name =  "xview_CC{}_Nxinstance-seed{}.xlsx".format(opt.cc_id, seed)          
    mode = 'w'
...
def computer_avg_all_seeds(seeds):
    #prefix = 'xview_CC{}_Nxinstance'
    
    eht = 'easy'
    apN = 50
    for cc_id in ccids:
        csv_dir = "result_output/1_cls/xview_CC+xview_BG/CC{}_Nxinstance/".format(cc_id)
#        if cc_id ==2:
#            prefix = 'synCC{}_ignore_notsolid_Nxquantity'.format(cc_id)
#        else:
#            prefix = 'synCC{}_Nxquantity'.format(cc_id)
        #prefix = 'xviewCC{}_Nxinstance'.format(cc_id)
        prefix = 'xview_CC{}_Nxinstance'.format(cc_id)
        cmb = []
        for seed in seeds:
