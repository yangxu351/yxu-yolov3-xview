...
#                p, r, ap, f1 = p[:, 0], r[:, 0], ap.mean(1), ap[:, 0]  # [P, R, AP@0.5:0.95, AP@0.5]
    ...
    if save_json and map and len(jdict):
        # fixme
        result_json_file = 'results_{}_epoch{}.json'.format(opt.name, opt.epoch)
...
def get_opt(dt=None, sr=None, comments=''):
    ...
    parser.add_argument("--epoch", type=int,  default=50, help="epoch 50, 100, 150, 200")
    ...
    for typ in eh_types:
        for i, cmt in enumerate(comments):
            ...
            for n in range(1, 4):
                ...
                rcvstr = endstr[:rcinx] # _RC*_v*
                vstr = rcvstr[rcvstr.find('v')+1:]
                VN = int(vstr)
                if cinx >= 0:
                    medix = cmt.find('_dyn')
                ...
                df_epoch.at[n, "#Num"] = n
                for epoch in range(150, 220, 5):
                    prefix = 'syn_AP{}_epoch{}'.format(apN, epoch)
...
#                    print(glob.glob(os.path.join(opt.weights_dir.format(opt.class_num, cmt, sd), '*_{}_seed{}'.format(hyp_cmt, sd), 'best_seed{}.pt'.format(sd))))
    #                all_weights = glob.glob(os.path.join(opt.weights_dir.format(opt.class_num, cmt, sd), '*_{}_seed{}'.format(hyp_cmt, sd), 'best_*seed{}.pt'.format(sd)))
                    all_weights = glob.glob(os.path.join(opt.weights_dir.format(opt.class_num, cmt, sd), '*_{}_seed{}'.format(hyp_cmt, sd), 'backup{}.pt'.format(epoch)))
                    all_weights.sort()
#                    print('all weights', all_weights)
                    opt.weights = all_weights[-1*n]
                    ...
                    df_epoch.at[n, "Seen"] = seen
                    df_epoch.at[n, "NT"] = nt     
                    df_epoch.at[n, "epoch{}".format(epoch)] = mapv
    ...
    #                mode = 'w'    
            ...
            with pd.ExcelWriter(os.path.join(csv_dir, csv_name), mode=mode) as writer:
                df_epoch.to_excel(writer, sheet_name='RC{}_AP{}_{}'.format(opt.rare_class, opt.apN, opt.type), index=False) # 
