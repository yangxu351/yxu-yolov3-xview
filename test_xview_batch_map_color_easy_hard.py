...
#    for ix, mk in enumerate(model_maps.keys()):
    ...
    #                    print('\n neu_correct: {}  pred[:,4]:{}  pred[:, 5]:{} tcls:{}'.format(neu_correct, pred[:, 4].cpu(), pred[:, 5].cpu(), tcls))
            ...
            if len(stats):
                ...
                # print('*stats', *stats)
                # print('iou', iou[0])
                p, r, ap, f1, ap_class = ap_per_class(*stats, pr_path=opt.result_dir, pr_name= pr_name, pr_legend=pr_legend, rare_class=opt.rare_class, apN=iou)
                ...
                mp, mr, mapv, mf1 = p.mean(), r.mean(), ap.mean(), f1.mean()
...
#        df_pr_ap = pd.DataFrame(columns=["Version","Seen", "NT", "AP{}".format(ap_list[0]), "AP{}".format(ap_list[1]), "AP{}".format(ap_list[2])])
        df_pr_ap = pd.DataFrame(columns=["Version","Seen", "NT"] + ["AP{}".format(x) for x in iou_list] + ["mAP[0.2:0.5]"])
...
#            print(glob.glob(os.path.join(opt.weights_dir.format(opt.class_num, cmt, sd), "*_{}_seed{}".format(hyp_cmt, sd), "best_seed{}.pt".format(sd))))
        ...
        csv_dir = "result_output/{}_cls/{}/".format(opt.class_num, cmt[:einx] + '_RC{}'.format(opt.rare_class))
...
#    base_version = 30
#    for ix, pro in enumerate(pros[:1]):
