...
#    for ix, mk in enumerate(model_maps.keys()):
        ...
        for ox in range(len(iouv)):
            ...
            s = ('%20s' + '%10s' * 6) % ('Class', 'Images', 'Targets', 'P', 'R', 'AP@{:.2f}'.format(iou[0]), 'F1')
            p, r, f1, mp, mr, mapv, mf1 = 0., 0., 0., 0., 0., 0., 0.
            loss = torch.zeros(3)
            ...
            for batch_i, (imgs, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
                ...
                for si, pred in enumerate(output):
                    ...
                    neu_correct = torch.zeros(len(pred), 1, dtype=torch.bool)
                    if nl:
                        ...
                        if len(pi) and len(ti) and not len(ni):
                            ...
                            for j in (ious > iouv[0]).nonzero():
                                ...
                                if d not in detected:
                                    detected.append(d)
                                    correct[pi[j]] = ious[j] > iouv[0]  # iou_thres is 1xn
    ...
    #                            print('s --------> ', s)
                                ...
                                if d not in detected:
                                    detected.append(d)
                                    neu_correct[pi[s]] = neu_ious[s] > iouv[0]  # iou_thres is 1xn
                                    if len(detected) == nl:  # all targets already located in image
                                        break
    ...
    #                    print('\n neu_correct: {}  pred[:,4]:{}  pred[:, 5]:{} tcls:{}'.format(neu_correct, pred[:, 4].cpu(), pred[:, 5].cpu(), tcls))
            ...
            if len(stats):
                ...
                # print('*stats', *stats)
                p, r, ap, f1, ap_class = ap_per_class(*stats, pr_path=opt.result_dir, pr_name= pr_name, pr_legend=pr_legend, rare_class=opt.rare_class, apN=iou[0])
    ...
    return seen, nt.sum(), mp_arr, mr_arr, map_arr, mf1_arr
...
#            opt.data = "data_xview/{}_cls/{}/xview_rc_test_{}_m{}_rc{}_{}.data".format(opt.class_num, base_cmt, base_cmt, opt.model_id, opt.rare_class, opt.type)
            opt.result_dir = opt.result_dir.format(opt.class_num, cmt, sd, "test_on_xview_{}_m{}_rc{}_map_{}".format(hyp_cmt, opt.model_id, opt.rare_class, opt.type))
            opt.data = "data_xview/{}_cls/{}/xview_rc_test_{}_m{}_rc{}_{}.data".format(opt.class_num, base_cmt, base_cmt, opt.model_id, opt.rare_class, opt.type)
...
#                rs_row = {"Version":VN, "Seen":seen, "NT":nt, "Precision":mp, "Recall":mr, "AP@{}".format(apN):mapv, "F1":mf1}
#                df_pr_ap.append(rs_row, ignore_index=True)
            df_pr_ap.at[ix, "Version"] = VN
            df_pr_ap.at[ix, "Seen"] = seen
            df_pr_ap.at[ix, "NT"] = nt
            for ux in range(len(iou_list)):
                df_pr_ap.at[ix, "AP{}".format(iou_list[ux])] = mapv[0, ux]
            df_pr_ap.at[ix, "mAP[0.2:0.5]"] = mapv.mean(axis=1)[0]
...
#        base_version = 21
#        if color_pro == 0:
#            cmt = "syn_xview_bkg_px15whr3_xbw_xbkg_unif_mig21_shdw_scatter_uniform_50_angle_rnd_color_bias0_model4_v{}_color".format(base_version)
#        else:
#            cmt = "syn_xview_bkg_px15whr3_xbw_xbkg_unif_mig21_shdw_scatter_uniform_50_angle_rnd_color_bias{}_model4_v{}_color".format(color_pro*25.5, color_pro+base_version)           
#        comments.append(cmt)
#    model_id = 4
#    rare_cls = 1
...
#        base_version = 1
#        if color_pro == 0:
#            cmt = "syn_xview_bkg_px23whr3_xbsw_xwing_scatter_gauss_30_bias0_model1_v{}_color".format(color_pro+base_version)
#        else:
#            cmt = "syn_xview_bkg_px23whr3_xbsw_xwing_scatter_gauss_30_color_bias{}_model1_v{}_color".format(color_pro*25.5, color_pro+base_version)           
#        comments.append(cmt)
#        
#    model_id = 1
#    rare_cls = 2   
...
#        base_version = 1
#        if color_pro == 0:
#            cmt = "syn_xview_bkg_px23whr3_xbw_xcolor_xbkg_unif_shdw_scatter_gauss_40_bias0_model5_v{}_color".format(base_version)
#        else:
#            cmt = "syn_xview_bkg_px23whr3_xbw_xcolor_xbkg_unif_shdw_scatter_gauss_40_color_bias{}_model5_v{}_color".format(color_pro*25.5, color_pro+base_version)             
#        comments.append(cmt)
#        
#    model_id = 5
#    rare_cls = 3
...
#        base_version = 12
#        if color_pro == 0:
#            cmt = "syn_xview_bkg_px23whr3_xbw_xcolor_xbkg_unif_shdw_split_scatter_gauss_1_color_bias0_model5_RC4_v{}_color".format(base_version)
#        else:
#            cmt = "syn_xview_bkg_px23whr3_xbw_xcolor_xbkg_unif_shdw_split_scatter_gauss_1_color_bias{}_model5_RC4_v{}_color".format(color_pro*25.5, color_pro+base_version)             
#        comments.append(cmt)
#        
#    model_id = 5
#    rare_cls = 4
...
#        base_version = 1
#        if color_pro == 0:
#            cmt = "syn_xview_bkg_px15whr3_xbw_xcolor_xbkg_unif_shdw_scatter_gauss_50_color_bias0_model5_RC5_v{}_color".format(color_pro+base_version)
#        else:
#            cmt = "syn_xview_bkg_px15whr3_xbw_xcolor_xbkg_unif_shdw_scatter_gauss_50_color_bias{}_model5_RC5_v{}_color".format(color_pro*25.5, color_pro+base_version)             
#        comments.append(cmt)
#        
#    model_id = 5
#    rare_cls = 5 
...
#        cmt = 'syn_xview_bkg_px15whr3_xbw_xbkg_unif_mig21_shdw_scatter_gauss_2_7_rndsolar_dynmu_size_bias{}_RC1_v{}_color'.format(pro*size_base, ix+ base_version)
#        comments.append(cmt)
#    main()
#
#    '''RC2''' 
...
#    base_version = 70
#    for ix, pro in enumerate(pros):
#        cmt = 'syn_xview_bkg_px15whr3_xbw_xbkg_unif_mig21_shdw_scatter_gauss_2_7_rndsolar_dynsigma_size_bias{}_RC1_v{}_color'.format(pro*size_base, ix+ base_version)
#        comments.append(cmt)
#    main()
#
#    '''RC2''' 
...
#    base_version = 50
#    for ix, pro in enumerate(pros):
#        cmt = 'syn_xview_bkg_px23whr3_xbsw_xwing_xbkg_shdw_split_scatter_gauss_rndsolar_dynsigma_size_bias{}_RC2_v{}_color'.format(pro*size_base, ix+ base_version)
#        comments.append(cmt)
#    main()
#
#    '''RC3''' 
...
#    base_version = 50
#    for ix, pro in enumerate(pros):
#        cmt = 'syn_xview_bkg_px23whr3_xbw_xbkg_unif_shdw_split_scatter_gauss_rndsolar_dynsigma_size_bias{}_RC3_v{}_color'.format(pro*size_base, ix+ base_version)
#        comments.append(cmt)          
#    main()
#
#    '''RC4''' 
...
#    base_version = 50
#    for ix, pro in enumerate(pros):
#        cmt = 'syn_xview_bkg_px23whr3_xbw_xcolor_xbkg_unif_shdw_split_scatter_gauss_rndsolar_dynsigma_size_bias{}_RC4_v{}_color'.format(pro*size_base, ix+ base_version)
#        comments.append(cmt)   
#    main()
    ...
    base_version = 40
    for ix, pro in enumerate(pros):
        cmt = 'syn_xview_bkg_px23whr3_xbw_xcolor_xbkg_unif_shdw_split_scatter_gauss_rndsolar_dynsigma_size_bias{}_RC5_v{}_color'.format(pro*size_base, ix+ base_version)
        comments.append(cmt)   
    main()
