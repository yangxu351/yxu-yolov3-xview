...
if __name__ == "__main__":
    ...
    ccids = [1, 2] # 1, 2, 3, 4, 5
    far_thres = 3
    cc_ratios = [4]  # 1, 2,3,4,5,6
    seeds = [0]# , 1, 2
    typ = "easy" # "hard",
    for ccs in cc_ratios:
        for seed in seeds:
            df_pr_ap_far = pd.DataFrame(columns=["CC", "Seen", "NT", "AP{}".format(apN), "Pd(FAR=0.25)", "Pd(FAR=0.5)", "Pd(FAR=1)", "Precision", "Recall" , "F1"]) #, "Precision", "Recall" , "F1"
    ...
    #            opt.type = "hard"
                ...
                opt.data = 'data_xview/{}_cls/{}/CC/xview_nccbkg_aug_cc{}_test_{}_{}.data'.format(opt.class_num, base_cmt, opt.cc_id, base_cmt, opt.type)
    ...
    #            print(os.path.join(opt.weights_dir.format(opt.class_num, cmt, sd), "*_{}_seed{}".format(hyp_cmt, sd), "best_seed{}.pt".format(sd)))
            ...
            csv_name =  "{}_CC_AP{}_{}_seed{}.xls".format(tif_name, apN, opt.type, seed)
