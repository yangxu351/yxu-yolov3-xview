...
#                    print('\n neu_correct: {}  pred[:,4]:{}  pred[:, 5]:{} tcls:{}'.format(neu_correct, pred[:, 4].cpu(), pred[:, 5].cpu(), tcls))
    ...
    last_file.write('%d %d %d %1.6f %1.6f %1.6f %1.6f\n' % (opt.epochs -1, seen, nt.sum(), mp, mr, map, mf1))    
    print(pf % ('all', seen, nt.sum(), mp, mr, map, mf1))
     # Return results
...
def get_opt(dt=None, sr=None, comments=''):
    ...
    parser.add_argument('--epochs', type=int, default=220, help='number of epochs')  # 60 6
...
#    prefix = 'syn_lr0.001'
    ...
    for typ in eh_types:
        for cmt in comments:
            last_file_name = '{}_iou{}_last_{}.txt'.format(cmt, apN, typ)
            last_file = open(os.path.join('result_output/1_cls/{}_seed{}/'.format(cmt, sd), last_file_name), 'w')

            prefix = 'syn_iou{}_last'.format(apN)
            ...
            opt.result_dir = opt.result_dir.format(opt.class_num, cmt, sd, 'test_on_xview_{}_m{}_rc{}_{}_iou{}_last'.format(hyp_cmt, opt.model_id, opt.rare_class, opt.type, apN))
...
#            print(glob.glob(os.path.join(opt.weights_dir.format(opt.class_num, cmt, sd), '*_{}_seed{}'.format(hyp_cmt, sd), 'best_seed{}.pt'.format(sd))))
            all_weights = glob.glob(os.path.join(opt.weights_dir.format(opt.class_num, cmt, sd), '*_{}_seed{}'.format(hyp_cmt, sd), 'last*.pt'))
            all_weights.sort()
