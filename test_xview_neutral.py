...
from utils.utils_xview_neutral import ap_per_class, plot_roc_easy_hard
...
def test(cfg,
    ...
    if dataloader is None:
        if opt.model_id is not None or opt.rare_class is not None:
...
#                    print('detected ', detected)
    ...
    if len(stats):
        pr_name= 'xview_only_iou@{} conf_thres{}'.format(iouv[0], conf_thres) # + ' @IoU: {:.2f} '.format(iouv[0]) + ' conf_thres: {} '.format(conf_thres)
        pr_legend = 'xview_rc{}bkg{}'.format(opt.rc_batch_size, opt.batch_size-opt.rc_batch_size)
        # print('*stats', *stats)
        ...
        # plot_roc_easy_hard(*stats, pr_path=opt.result_dir, pr_name= pr_name, pr_legend=pr_legend, rare_class=opt.rare_class, area=area, ehtype=opt.type)
        # if niou > 1:
        ...
        nt = torch.zeros(1)
    #     if len(stats):
    #     p, r, ap, f1, ap_class = a(*stats, pr_path=opt.result_dir, pr_name=opt.name)
    #     # if niou > 1:
    #     #       p, r, ap, f1 = p[:, 0], r[:, 0], ap[:, 0], ap.mean(1)  # average across ious
    #     #fixme --yang.xu
    #     if niou > 1:
