...
#                    print('\n neu_correct: {}  pred[:,4]:{}  pred[:, 5]:{} tcls:{}'.format(neu_correct, pred[:, 4].cpu(), pred[:, 5].cpu(), tcls))
    ...
    rs_row = {"Version":VN, "Seen":seen, "NT":nt.sum(), "Precision":mp_arr.mean(), "Recall":mr_arr.mean(), "AP@{}".format(apN):map_arr.mean(), "F1":mf1_arr.mean()}
...
#    prefix = "syn_px30"
    ...
    for typ in eh_types:
        ...
        for ix, cmt in enumerate(comments):
            ...
            cinx = cmt.find("RC") # first letter index RC2_v{}_color
            endstr = cmt[cinx:]
...
#            print(glob.glob(os.path.join(opt.weights_dir.format(opt.class_num, cmt, sd), "*_{}_seed{}".format(hyp_cmt, sd), "best_seed{}.pt".format(sd))))
        ...
        csv_dir = "result_output/{}_cls/{}/".format(opt.class_num, cmt[:cmt.find("bias")+4] + cmt[cmt.find("_RC"):cmt.find("_v")])
