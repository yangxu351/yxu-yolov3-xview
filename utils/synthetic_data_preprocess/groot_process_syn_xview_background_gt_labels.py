...
def group_object_annotation_and_draw_bbox(dt, px_thresh=20, whr_thres=4):
    ...
    '''
    syn_args.syn_data_dir = syn_args.syn_data_dir.format(dt)
    step = syn_args.tile_size * syn_args.resolution
...
def get_annos_from_lbl_image_and_draw_bbox(dt, px_thresh=20, whr_thres=4):
    ...
    gt_files = np.sort(glob.glob(os.path.join(lbl_path, '*{}'.format(IMG_FORMAT))))
    bbox_folder_name = 'px{}whr{}_{}_all_annos_with_bbox_step{}'.format(px_thresh, whr_thres, dt, step)
    save_bbx_path = os.path.join(syn_args.syn_box_dir, bbox_folder_name)
    if not os.path.exists(save_bbx_path):
        os.makedirs(save_bbx_path)
    else:
...
def split_syn_xview_background_trn_val(seed=17, comment='syn_color', pxwhr='px23whr3'):
    ...
    data_txt_dir = syn_args.syn_txt_dir.format(comment)
...
def split_trn_val_for_syn_and_real(seed=17, comment='wnd_syn_real', pxwhr='px23whr3', real_img_dir='', real_lbl_dir='', dt='color'):
    syn_args.syn_data_dir = syn_args.syn_data_dir.format(dt)
    step = syn_args.tile_size * syn_args.resolution
...
def create_syn_and_real_data(comment='wnd_syn_real', seed=17, dt='color'):
    data_txt_dir = syn_args.syn_txt_dir.format(comment)
...
def get_args(cmt=''):
    ...
    parser.add_argument("--syn_annos_dir", type=str, default='/data/users/yang/data/synthetic_data/{}_txt_xcycwh/',
                        help="syn label.txt")
    parser.add_argument("--syn_box_dir", type=str, default='/data/users/yang/data/synthetic_data/{}_gt_bbox/',
                        help="syn box files")
    parser.add_argument("--syn_txt_dir", type=str, default='/data/users/yang/code/yxu-yolov3-xview/data_wnd/{}',
    ...
    parser.add_argument("--link_r", type=int, default=5,
...
#    cmt = 'syn_uspp_bkg_shdw_split_scatter_gauss_rotate_rnd_ort_wnd_v3'
    cmt = 'syn_uspp_ca_az_bkg_shdw_split_scatter_gauss_rotate_rnd_ort_wnd_v4'
...
#        base_pxwhrs = base_pxwhrs.format(seed)
        split_syn_xview_background_trn_val(seed, comment, pxwhr)
...
#    real_img_dir = 'data/users/yang/data/wind_turbine_images/'
#    real_lbl_dir = 'data/users/yang/data/wind_turbine_labels/'
#    for cmt in comments:
#        base_pxwhrs = base_pxwhrs.format(seed)
